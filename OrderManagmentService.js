...
async function sendMessage() {
  try {
    ...
    const exchange = 'order_service'; // Set the exchange name
    ...
    await channel.bindQueue(invoice, exchange, "order_service");
    await channel.bindQueue(shipping, exchange, "order_service");
    await channel.bindQueue(warehouse, exchange, "order_service");
    await channel.bindQueue(payment, exchange, "order_service");

    // Send a message to the exchange with a routing key
    channel.publish(exchange, "order_service", Buffer.from(message));
